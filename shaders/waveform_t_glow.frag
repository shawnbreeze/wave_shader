...
const float GLOW_SIZE = 0.0005; // Размер ореола
const float GLOW_INTENSITY = 0.25; // Интенсивность ореола
const vec4 GLOW_COLOR = vec4(0.04, 0.84, 1, 1); // Цвет ореола (голубоватый)
const float GLOW_DISTANCE = 0; // Расстояние от волны, на котором будет свечение
...
// Функция для создания мягкого ореола вокруг волны с заданным расстоянием от волны
float createGlow(float distance, float glowDistance, float glowSize, float glowIntensity) {
    // Вычисляем смещенное расстояние (приближаемся к максимуму когда distance = glowDistance)
    float offsetDistance = abs(distance - glowDistance);
    return glowIntensity * exp(-offsetDistance * offsetDistance / glowSize);
}
...
void main()
{
    ...
    // Размер пикселя по вертикали для корректного сглаживания
    float pixelHeight = 1.0 / u.resolution.y;
    float effectiveSmoothing = max(u.smoothing, pixelHeight * 0.5);
    ...
    // Рассчитываем расстояние до ближайшей точки волны для создания ореола
    float distToRightWave = min(abs(y - rTop), abs(y - rBot));
    float distToLeftWave = min(abs(y - lTop), abs(y - lBot));
    float distToWave = min(distToRightWave, distToLeftWave);
    
    // Создаем ореол с учетом расстояния GLOW_DISTANCE
    float glow = createGlow(distToWave, GLOW_DISTANCE, GLOW_SIZE, GLOW_INTENSITY);
    
    // Смешиваем цвета с учётом сглаживания и ореола
    // Сначала применяем ореол к фону с собственным цветом ореола
    vec4 glowColor = mix(u.backColor, GLOW_COLOR, glow);
    
    // Затем добавляем саму волну поверх
    FragColor = mix(glowColor, u.waveColor, clamp(waveR + waveL, 0.0, 1.0));
}
