...
const float baseCenter = 0.5; // Центральное положение волны вместо левого и правого
...
void main()
{
    ...
    // Вычисляем усредненные значения для моно отображения
    float monoMin = (lMin + rMin) / 2.0;
    float monoMax = (lMax + rMax) / 2.0;
    
    // Вычисляем границы моно волны (по центру)
    float monoTop = baseCenter + (monoMax * u.ampScale * 2);
    float monoBot = baseCenter + (monoMin * u.ampScale * 2);
    ...
    // Используем улучшенную функцию для более стабильного отображения моно волны
    float monoWave = smoothstep(monoBot - effectiveSmoothing, monoBot, y) - 
                     smoothstep(monoTop, monoTop + effectiveSmoothing, y);

    // Смешиваем цвета с учётом сглаживания для моно волны
    FragColor = mix(u.backColor, u.waveColor, clamp(monoWave, 0.0, 1.0));
}
